...
pub fn view() -> Element {
    ...
    let current_error = btc_sign_transaction
        .read()
        .send_transaction
        .as_ref()
    ...
    let on_submit = move |_| {
        ...
        let b39 = bip39_val();
        let last_v = btc_ctx.btc_modal.read().last_view;
        ...
        match mode.as_str() {
            ...
            _ => {
                validation_error = Some("ERR: INVALID_MODE".to_string());
            }
        }
        ...
        // --- EXECUTE ---
        tokio::spawn(BtcSendLogic::process(BtcSendParams {
            mode,
            passphrase: p_val,
            mnemonic: seed_string,
            bip39_pass: b39,
            recipient,
            amount,
            fee,

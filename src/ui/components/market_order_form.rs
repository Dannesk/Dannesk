...
pub fn MarketOrderForm(
    ...
    on_pair_change: EventHandler<()>,
) -> Element {
    let price_f = price_sig().parse::<f64>().unwrap_or(0.0);
    let deviation = if market_rate > 0.0 {
        ((price_f - market_rate) / market_rate) * 100.0
    } else {
        0.0
    };
    let abs_dev = deviation.abs();
    let dir = if deviation < 0.0 { "below" } else { "above" };
    ...
    rsx! {
        style { {r#"
            ...
            .terminal-input-wrapper { display: grid; grid-template-columns: auto 1fr auto; align-items: center; cursor: pointer; background: var(--input-bg);  border: 1px solid var(--border); padding: 0.8rem 1rem; }
            .bracket { color: var(--text-secondary); opacity: 0.4; font-weight: bold; }
            .inner-input { background: transparent; border: none; outline: none; cursor: text; color: var(--text); font-family: inherit; font-size: 1rem; padding: 0 1rem; width: 100%; }
            ...
            .search-item-btn { text-align: left; background: var(--selection); padding: 0.75rem 1rem; cursor: pointer; background: var(--input-bg); color: var(--text); border: none; border-bottom: 1px solid var(--border); font-family: inherit; }
            ...
            .toggle-btn { padding: 0.7rem 0; font-size: 0.75rem; cursor: pointer; font-family: inherit; background: transparent; border: 1px solid var(--border); color: var(--text-secondary); text-align: center; }
            .toggle-btn-active { border-color: var(--accent); color: var(--accent); background: var(--selection); }
            .market-rate-container { display: grid; place-items: center; margin: 1rem 0; }
            .market-rate-readout { font-size: 0.7rem; color: var(--text-secondary); margin-top: 0.5rem; letter-spacing: 1px; white-space: nowrap; }
            .action-footer { display: grid; grid-template-columns: 1fr; margin-top: 1rem; }
            .rate-row { display: flex; justify-content: space-between; align-items: center; margin: 1.5rem 0; flex-wrap: nowrap; }
        "#} }
        ...
        div { class: "trade-step-container",
            ...
            div { class: "input-section",
                ...
                if has_selection && !is_searching() {
                    button {
                        ...
                        span { class: "bracket", "[" }
                        span { style: "color: var(--accent); text-align: center;", "{selected_base()}", " / ", "{selected_quote()}" }
                        span { class: "bracket", "]" }
                    }
                } else {
                    div {
                            ...
                            div { class: "search-results-box",
                                for (pair, b, q) in TRADING_PAIRS.iter().filter(|(_, b, _)| b.to_lowercase().contains(&search_query().to_lowercase())) {
                                    button {
                                        ...
                                        onclick: move |_| {
                                            let old_base = selected_base();
                                            let old_quote = selected_quote();
                                            selected_base.set((*b).to_string());
                                            selected_quote.set((*q).to_string());
                                            if old_base != selected_base() || old_quote != selected_quote() {
                                                on_pair_change.call(());
                                            }
            ...
            div { class: "input-section row-grid",
                ...
                div {
                    div { class: "input-label-row",
                        div { class: "input-label",
                            if selected_base().is_empty() {
                                "LIMIT_PRICE (PER UNIT)"
                            } else {
                                "LIMIT_PRICE (PER {selected_base()})"
                            }
                        }
                    }
                    div { class: "terminal-input-wrapper",
                    ...
                    if is_manual() && abs_dev > 1.0 {
                        div { class: "market-rate-readout",
                            "Note: Your price is {abs_dev:.2}% {dir} current market."
                        }
                    }
                }
            }
            ...
            div { class: "input-section",
                div { class: "input-label-row", div { class: "input-label", "OFFER (%)" } }
            ...
            {
                let amount_f = amount_sig().parse::<f64>().unwrap_or(0.0);
                let total_f = amount_f * price_f;
                if has_selection {
                    rsx! {
                        div { class: "rate-row",
                            if market_rate > 0.0 {
                                div { class: "market-rate-readout",
                                    "CURRENT MARKET RATE: 1 {selected_base()} ≈ {market_rate:.4} {selected_quote()}"
                                }
                            }
                            if total_f > 0.0 {
                                div { class: "market-rate-readout",
                                    "EST. TOTAL COST: {total_f:.4} {selected_quote()}"
                                }
                            }
                        }
                    }

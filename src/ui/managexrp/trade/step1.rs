...
pub fn view() -> Element {
    ...
    let mut is_manual = use_signal(|| false);
    ...
    let market_rate = use_memo(move || {
        ...
        if b_raw.is_empty() || q_raw.is_empty() {
            return 0.0;
        }
    ...
    let mut update_price = move |pct: f64| {
        let rate = market_rate();
        if rate > 0.0 {
            let per_unit = rate * (1.0 + (pct / 100.0));
            price_sig.set(format!("{:.6}", per_unit));
        }
    };

    use_effect(move || {
        if !is_manual() && has_selection {
            update_price(active_pct());
        }
    });

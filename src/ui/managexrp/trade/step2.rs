...
pub fn view() -> Element {
    ...
    let summary_rows = vec![
        ...
        (
            ...
            format!("{limit_price} {quote_asset}"),
        ),
        (
            "TOTAL_COST".to_string(),
            format!(
                "{:.4} {quote_asset}",
                amount.parse::<f64>().unwrap_or(0.0) * limit_price.parse::<f64>().unwrap_or(0.0)
            ),
        ),
        ("PERCENTAGE".to_string(), fee_display),
    ...
    render_send_review(
        ...
        summary_rows,
        ">> ATTENTION: VERIFY ORDER DETAILS CAREFULLY. ENSURE TRADE IS CORRECT.".to_string(),

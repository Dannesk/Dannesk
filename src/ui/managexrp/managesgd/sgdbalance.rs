...
pub fn render_sgd_balance() -> Element {
    ...
    let (sgd_amount, _, sgd_limit) = *sgd_ctx.sgd.read();
    ...
    let (int_part, frac_part) = if hide_balance {
        ...
        (
            add_commas(total_sgd.floor() as i64),
            format!(".{:02}", (total_sgd.fract() * 100.0).round() as i64),
        )
    };

    let formatted_raw_amount = if hide_balance {
        "****".to_string()
    } else {
        format!("{:.6}", sgd_amount)
    };

use crate::channel::{CHANNEL, ProgressState, BalanceActiveView};
...
impl PinLogic {
    pub async fn change_pin(old_pin: String, new_pin: String) {
        ...
        let result =
            tokio::task::spawn_blocking(move || crate::pin::change_pin(&old_pin, &new_pin)).await;
        ...
        match result {
            Ok(Ok(())) => {
                ...
                let _ = CHANNEL.balance_view_tx.send(BalanceActiveView::Main);
            }
            _ => {
                // Failure
                let _ = CHANNEL.progress_tx.send(Some(ProgressState {

use crate::channel::{CHANNEL, ProgressState, TransactionState, WSCommand};
use crate::bridge::json_storage::{self, get_config_path, remove_json};
...
impl WalletOperations {
    ...
    pub async fn delete_key(wallet_address: String) {
        ...
        let _ = CHANNEL.progress_tx.send(Some(ProgressState {
            progress: 1.0,
            message: if delete_success {
                "XRP Key deletion complete".to_string()
            } else {
                "Deletion finished with errors".to_string()
            },
        }));
    }
    ...
    pub async fn remove_wallet(wallet_address: String, ws_tx: Sender<WSCommand>) {
        ...
        let _ = CHANNEL.progress_tx.send(Some(ProgressState {
            progress: 0.5,
            message: "Local files removed, clearing memory..".to_string(),
        }));
        ...
        // 4. Reset UI/Channels (XSGD now included ✅)
        ...
        let _ = CHANNEL.sgd_tx.send((0.0, false, None));

...
pub async fn construct_blob(
    ...
    // Default to a high limit if not provided (for creation)
    let trustline_limit_value = cmd.trustline_limit.clone().unwrap_or_else(|| "1000000".to_string());
    
    // We need an identifier to know which trustline to create/delete.
    ...
    let trustline_limit = IssuedCurrencyAmount {
        ...
        value: Cow::Owned(trustline_limit_value.clone()),
    };

    // Determine flags based on whether this is creation or deletion
    let flags = if trustline_limit_value == "0" {
        vec![TrustSetFlag::TfClearNoRipple].into()
    } else {
        vec![TrustSetFlag::TfSetNoRipple].into()
    };

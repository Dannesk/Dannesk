...
use crate::ws::config::{MAX_RECONNECT_ATTEMPTS, RECONNECT_BACKOFF_SECONDS, RELAY_WS_URL};
    ...
    pub async fn next_message(&mut self) -> Option<Result<Message, String>> {
        ...
        if let Some(ref mut ws) = self.ws_stream {
            match timeout(Duration::from_secs(90), ws.next()).await {
                ...
                Ok(Some(Ok(Message::Pong(data)))) => Some(Ok(Message::Pong(data))),
                    ...
                    match serde_json::from_str::<WsMessage>(&text) {
                        ...
                        Err(_) => {
                            if serde_json::from_str::<Value>(&text).is_ok() {
                                ...
                                Some(Err("Invalid JSON message".to_string()))
                            }
                        }
                    }
                }
                Ok(Some(Ok(message))) => Some(Ok(message)),
                Ok(Some(Err(e))) => {

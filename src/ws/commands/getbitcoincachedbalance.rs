    ...
    match message {
        Message::Text(text) => {
            ...
            if let Some(wallet) = data.get("wallet").and_then(|w| w.as_str()) {
                ...
                let _transaction_count = if let Some(transactions) =
                    data.get("transaction").and_then(|t| t.as_array())
                {
                ...
                if let Err(e) = CHANNEL.bitcoin_wallet_tx.send((
                    balance_btc,
                ...
                let transaction_state = BtcTransactionState {

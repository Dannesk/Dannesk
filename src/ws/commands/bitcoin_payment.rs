...
pub async fn construct_transaction(
    ...
    if change > 0 {
        let change_addr = Address::from_str(&wallet_obj.address)
    ...
    let private_key = PrivateKey::from_wif(&wallet_obj.private_key)
    ...
    for (i, _) in tx.input.iter().enumerate() {
        let prev_output_script =
            Address::p2wpkh(&compressed_public_key, bitcoin::Network::Bitcoin).script_pubkey();

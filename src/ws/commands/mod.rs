...
impl Command {
    ...
    pub async fn process_response(
        ...
        match self {
            ...
            Command::GetLedgerData => ledger::process_response(message, current_wallet)
                .await
                .map(|_| ()),
            Command::GetBalance => {
            ...
            Command::GetBitcoinLedgerData => {
                bitcoin_ledger::process_response(message, bitcoin_current_wallet)
                    .await
                    .map(|_| ())
            }
            Command::GetBTCBalance => {
